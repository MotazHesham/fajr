...
@can('said_about_us_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.said-aboutuses.create') }}">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-SaidAboutUs">
                ...
                <tbody>
                    @foreach($saidAboutuses as $key => $saidAboutUs)
                        <tr data-entry-id="{{ $saidAboutUs->id }}">
                            ...
                            <td>
                                @can('said_about_us_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.said-aboutuses.show', $saidAboutUs->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('said_about_us_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.said-aboutuses.edit', $saidAboutUs->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('said_about_us_delete')
                                    <form action="{{ route('admin.said-aboutuses.destroy', $saidAboutUs->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('said_about_us_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.said-aboutuses.massDestroy') }}",
